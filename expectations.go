...
// ExpectedSelectCtx is used to manage *sql.DB.SelectContext, *sql.Tx.SelectContext or *sql.Stmt.SelectContext expectations.
// Returned by *Sqlmock.ExpectExec.
type ExpectedSelectCtx struct {
	queryBasedExpectation
	result driver.Result
	delay  time.Duration
	dst    Value
...
// WillReturnError allows to set an error for expected database exec action
func (e *ExpectedSelectCtx) WillReturnError(err error) *ExpectedSelectCtx {
	e.err = err
	return e
}
...
// WillDelayFor allows to specify duration for which it will delay
// result. May be used together with Context
func (e *ExpectedSelectCtx) WillDelayFor(duration time.Duration) *ExpectedSelectCtx {
	e.delay = duration
	return e
}

// String returns string representation
func (e *ExpectedSelectCtx) String() string {
	msg := "ExpectedSelectCtx => expecting Select or ExecSelect which:"
	msg += "\n  - matches sql: '" + e.expectSQL + "'"
...
// to build a corresponding result. Or if actions needs to be tested against errors
// sqlmock.NewErrorResult(err error) to return a given error.
func (e *ExpectedSelectCtx) WillReturnResult(result driver.Result) *ExpectedSelectCtx {
	e.result = result
	return e
}
